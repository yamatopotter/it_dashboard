...
2. **Perf (concluído):** `incidents`, `timeline` e `report-builder` migrados para SQL (window functions `LAG` em `lib/incident-detection.ts` + agregações `FILTER` e stride em `lib/report-queries.ts`), todos validados por testes de integração. Não carregam mais o histórico completo em memória.
